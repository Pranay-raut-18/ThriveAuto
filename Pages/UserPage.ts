...
export class UserPage {
  ...
  constructor(page: Page) {
    ...
    this.statusDisableButton = page.locator(
      "//div[@class='MuiButtonBase-root MuiChip-root MuiChip-filled MuiChip-sizeSmall MuiChip-colorDefault MuiChip-deletable MuiChip-deletableColorDefault MuiChip-filledDefault css-7lzqhs']//*[name()='svg']"
    );
    ...
    this.userFirstRowRole = page.locator(
      '[data-rowindex="0"] div [title="Admin"]'
    );
    ...
    this.invalidLinkedInmessage = page.locator(
      'text="Please enter the LinkedIn URL in this format: https://linkedin.com/in/<unique identifier>"'
    );
    ...
    this.enterRoleMessage = page.locator(
   ...
   */
  async isSearchedAttributeDisplayedInUserList(
    value: string,
    colname: string
  ): Promise<boolean> {
    let oldCount: any = 0;
    ...
    do {
      oldCount = newCount;
      const userList = await this.userListing
        .locator(`[data-field="${colname}"]`)
        .all();
    ...
    const updatedUserList = await this.userListing
      .locator(`[data-field="${colname}"]`)
      .all();
    for (const user of updatedUserList) {
      ...
      if (
        userName &&
        userName.trim().toLowerCase().includes(value.toLowerCase())
      ) {
        return true;
      }
    }
  ...
  async isAllStausActive(value: string, datafield: string): Promise<boolean> {
    ...
    do {
      oldCount = newCount;
      const statusList = await this.userListing
    ...
    const fullStatusList = await this.userListing
      .locator(`[data-field="${datafield}"]`)
      .allInnerTexts();
  ...
  async isSearchBarHighlighted(): Promise<boolean> {
    ...
    const borderColor = await this.searchBar.evaluate(
  ...
  async getFilterValues(
    feild: string,
    allRoles = new Set<string>()
  ): Promise<string[]> {
    await this.page.waitForTimeout(1000);
    const filterValues = await this.userListing
      .locator(`[data-field="${feild}"]`)
      .allInnerTexts();
    filterValues.forEach((value) => allRoles.add(value));
    const size = allRoles.size;
    const lastElement = this.userListing
      .locator(`[data-field="${feild}"]`)
      .last();
  ...
  async enterCreateFeildValue(value: string): Promise<boolean | undefined> {
    ...
    const optVisible = await this.page
    ...
    if (optVisible == false) {
      return false;
    } else await this.page.getByRole("option", { name: value }).click();
  }
