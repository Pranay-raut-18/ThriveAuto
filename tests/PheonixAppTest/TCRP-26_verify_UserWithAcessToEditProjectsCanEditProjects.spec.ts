...
test("To Verify that the System role with view functionality is able to view people in the Pheonix Appplication", async ({
  ...
  //Verify edit button in jobs is visible
  await test.step("Checks if edit button in jobs is visible", async () => {
    const result = await pheonixAppPage.isEditButtoninJobsVisible();
